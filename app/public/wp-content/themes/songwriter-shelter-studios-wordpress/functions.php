        ...
        <div class="page--posts">
            ...
            <div class="forum-post newsfeed-well__background-color">
				...
				if (get_comments_number() == 0) {
				            ?>
						<a class="mobile-hide" href="<?php the_permalink();?>"><?php comments_number('Be the first to comment!', '1', '%')?></a><span class="mobile-show">0</span>
				                    <?php
				} else {
    ...
    ?>
                ...
                <?php }?>
                    <ul class="user-posts__list user-forum-posts" id="user-forum-posts">
                        ...
                        <li data-id="<?php the_ID();?>" data-post-type="<?php echo get_post_type(); ?>">
                            ...
                            <br>
                            <textarea readonly class="user-posts__content"><?php echo esc_attr(get_the_content()); ?>
                            </textarea>
                            <br>
        ...
        echo '<p>Please log in to see your posts.</p>';
            ...
            <div data-post-type="<?php echo $postType; ?>" class="user-create-posts user-posts-area__create" id="user-create-posts-area">
                ...
                <?php if (!is_user_logged_in()) {?>
                <div><i class="forum-button__caption forum-button__caption--log-in">Please log in to create a post.</i></div>
                <?php }?>
                <br>
                        ...
                        <div class="row">
                            <div class="row">
                                <div class="col-sm-4">
                                    ...
                                    <p class="italic__font smaller-font darker-color">
...
function pageArchiveBreadCrumb($args = [])
{
    ...
    ?>
                        <div class="page-links side-list__float-left <?php echo $args['float-right']; ?>">
                            ...
                            <div class="page-links__title page-links__main-background">
                                <a href="<?php echo get_post_type_archive_link($args['post-type']) ?>">
                                    <?php echo $args['title'] ?>
                                </a>
                            </div>
                            <ul class="min-list">
...
function forumPostMasterList($args = [])
{
    ...
    ?>
                            <div class="page-links side-list__float-left <?php echo $args['float-right']; ?> forum-breadcrumb__subsection">
                                <!-- Maybe link title of Side List to archive for the current post type -->
                                <div class="page-links__title page-links__main-background page-links__title--forum">
                                    <?php echo forumCategorySubtitle($args['query']->query['post_type']) . ' Posts'; ?>
                                </div>
                                <ul class="min-list">
                                    <?php
									while ($args['query']->have_posts()) {
...
function singleRelatedPost($argField, $title)
{
    ...
    if ($relatedField) {
        ...
        foreach ($relatedField as $field) {?>
                                        <li>
                                            <a class="inverse-link-color" href="<?php echo get_the_permalink($field); ?>">
                                                <?php echo get_the_title($field) ?>
                                            </a>
                                        </li>
                                        <?php }
        echo '</ul>';
...
function singleRelatedPostLinkBack($query, $title)
{
    if ($query->have_posts()) {
        ...
        while ($query->have_posts()) {
            $query->the_post();?>
                                        <li>
                                            <a class="inverse-link-color" href="<?php echo get_the_permalink(); ?>">
